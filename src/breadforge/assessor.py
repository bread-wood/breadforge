...
Before dispatching an agent, the assessor reads the issue and estimates task
complexity. The allocator maps the complexity tier to a concrete model, applying
confidence-based tier upgrades and respecting the BREADFORGE_MODEL override.
...
The estimator always uses the cheapest configured model to minimize overhead.
"""
...
class Allocator:
    ...
    def allocate(
        ...
        """Select a model for dispatch.

        Priority:
          1. BREADFORGE_MODEL env var (or override_model param) — no estimation needed
          2. Confidence < threshold → upgrade one tier
          3. Use estimate tier directly
        """
...
async def assess_and_allocate(
    ...
    *,
    ...
    env_override = override_model or os.environ.get("BREADFORGE_MODEL")
    if env_override:
        # Skip estimation entirely when model is pinned
